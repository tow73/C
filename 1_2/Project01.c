...
int setUnion(int set1[], int size1, int set2[], int size2, int setResult[]) //setA, sizeA, setB, sizeB, setC
{
	int i, j, c, empty;
	int k = 0;
	for (i = 0; i < size1; i++)
	...
	for (i = 0; i < c - 1; i++)
	{
		for (j = i + 1; j < c; j++)
		{
			if (setResult[i] > setResult[j])
			{
				empty = setResult[i];
				setResult[i] = setResult[j];
				setResult[j] = empty;
			}
		}
	}
...
int setIntersecton(int set1[], int size1, int set2[], int size2, int setResult[])
{
	...
	int k = 0;
	for (i = 0; i < size1; i++)
			if (hasElement(set2, size2, set1[i]) == 1)
			{
				setResult[k] = set1[i];
				k++;
			}
