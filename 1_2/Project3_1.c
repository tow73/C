...
void purchaseVideo(VideoInfo videoList[], int* videoCountPtr, char title[], int qty)  //항상 새로운 비디오  //char title[]이 더 나을 듯
{ 
	strcpy(videoList[*videoCountPtr].title, title);
	videoList[*videoCountPtr].qty = qty;
	(*videoCountPtr)++;
}
int searchVideoByTitle(VideoInfo videoList[], int videoCount, char title[])  //VideoInfo videoList[], char title[]
{
	int i;
	for (i = 0; i < videoCount; i++)
...
void rentVideo(VideoInfo videoList[], int videoCountPtr, RentInfo rentList[], int* rentCountPtr, char title[], int custId)  //char title[]
{
